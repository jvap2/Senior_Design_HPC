...
SU_Mem=np.divide(CPU,GPU_Mem_N)
SU_wo_Mem=np.divide(CPU,GPU_wo_Mem_N)
print(through)

fig, (ax1)= plt.subplots(1,1)
ax1.plot(size,CPU, 'k',label='CPU')
ax1.plot(size,GPU_Mem, 'b',label='Non-Tiled GPU total')
ax1.plot(size,GPU_wo_Mem,'r', label='Non-TiledGPU exec')
ax1.plot(size,GPU_Mem_N, 'b',marker='*',label='Tiled GPU total')
ax1.plot(size,GPU_wo_Mem_N,'r', marker='*',label='Tiled GPU exec')
ax1.set_xscale("log")
ax1.grid()
ax1.legend()
...
ax3.plot(size,SU_Mem,'b', label='Non-Tiled GPU total')
ax3.plot(size,SU_wo_Mem,'r', label='Non-Tiled GPU exec')
ax3.plot(size,SU_Mem,'b', marker='*', label='Tiled GPU total')
ax3.plot(size,SU_wo_Mem,'r', marker='*',label='Tiled GPU exec')
ax3.set_xscale("log")
ax3.grid()
ax3.legend()
