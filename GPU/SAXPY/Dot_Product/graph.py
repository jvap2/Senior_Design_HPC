...
DH=data['Memory Transfer (Device to Host)(ms)'].to_numpy()
HD=data['Memory Transfer (Host to Device)(ms)'].to_numpy()
...
ax1.plot(size,GPU_Mem, 'b',label='GPU total')
ax1.plot(size,GPU_wo_Mem, 'r',label='GPU exec')
ax1.set_xscale("log")
ax1.grid()
ax1.legend()
...
ax3.plot(size,SU_Mem,'b', label='GPU total')
ax3.plot(size,SU_wo_Mem,'k', label='GPU exec')
ax3.set_xscale("log")
ax3.grid()
ax3.legend()
