...
//    size_t vector_size_bytes = sizeof(int) * DATA_SIZE;
	...
	CImg<unsigned char> imgRGB = CImg<>("/home/sureshm/Senior_Design_HPC/GPU/ImageProcessing/Gray_Scale/grey_power.jpg");
	unsigned char* ptrRGB = imgRGB.data();
	CImg<unsigned char>imgGrayScale(imgRGB.width()-2,imgRGB.height()-2);
	unsigned char* ptrGray = imgGrayScale.data();
	size_t rgbSize = imgRGB.width() * imgRGB.height() *sizeof(unsigned char);
	size_t graySize = (imgRGB.width()-2) * (imgRGB.height()-2) * 1 * sizeof(unsigned char);
	int stride =(imgRGB.width()-2) * (imgRGB.height()-2);
	int h=imgRGB.height();
	int w =imgRGB.width();
	int gsize=(((h-2)/32)+1)*(w-2);
	int last_size=((h-2)-(((h-2)/32)+1)*31)*(w-2);
	std::cout<<(((h-2)/32)+1)<<std::endl;
	unsigned int gsize_b=(gsize)*sizeof(unsigned char);
	unsigned int last_size_b=(last_size)*sizeof(unsigned char);
	...
	unsigned char ptrg32[last_size]{};
	for (int j=0; j<(((h-2)/32)+1); j++){
		for(int k=0; k<(w-2);k++){
			ptrg1[j*(w-2)+k]=ptrGray[j*(w-2)+k];
			ptrg2[j*(w-2)+k]=ptrGray[j*(w-2)+k+ gsize];
			ptrg3[j*(w-2)+k]=ptrGray[j*(w-2)+k+ 2*gsize];
			ptrg4[j*(w-2)+k]=ptrGray[j*(w-2)+k+3*gsize];
			ptrg5[j*(w-2)+k]=ptrGray[j*(w-2)+k+4*gsize];
			ptrg6[j*(w-2)+k]=ptrGray[j*(w-2)+k+5*gsize];
			ptrg7[j*(w-2)+k]=ptrGray[j*(w-2)+k+6*gsize];
			ptrg8[j*(w-2)+k]=ptrGray[j*(w-2)+k+7*gsize];
			ptrg9[j*(w-2)+k]=ptrGray[j*(w-2)+k+8*gsize];
			ptrg10[j*(w-2)+k]=ptrGray[j*(w-2)+k+ 9*gsize];
			ptrg11[j*(w-2)+k]=ptrGray[j*(w-2)+k+ 10*gsize];
			ptrg12[j*(w-2)+k]=ptrGray[j*(w-2)+k+11*gsize];
			ptrg13[j*(w-2)+k]=ptrGray[j*(w-2)+k+12*gsize];
			ptrg14[j*(w-2)+k]=ptrGray[j*(w-2)+k+13*gsize];
			ptrg15[j*(w-2)+k]=ptrGray[j*(w-2)+k+14*gsize];
			ptrg16[j*(w-2)+k]=ptrGray[j*(w-2)+k+15*gsize];
			ptrg17[j*(w-2)+k]=ptrGray[j*(w-2)+k+16*gsize];
			ptrg18[j*(w-2)+k]=ptrGray[j*(w-2)+k+ 17*gsize];
			ptrg19[j*(w-2)+k]=ptrGray[j*(w-2)+k+ 18*gsize];
			ptrg20[j*(w-2)+k]=ptrGray[j*(w-2)+k+19*gsize];
			ptrg21[j*(w-2)+k]=ptrGray[j*(w-2)+k+20*gsize];
			ptrg22[j*(w-2)+k]=ptrGray[j*(w-2)+k+21*gsize];
			ptrg23[j*(w-2)+k]=ptrGray[j*(w-2)+k+22*gsize];
			ptrg24[j*(w-2)+k]=ptrGray[j*(w-2)+k+23*gsize];
			ptrg25[j*(w-2)+k]=ptrGray[j*(w-2)+k+24*gsize];
			ptrg26[j*(w-2)+k]=ptrGray[j*(w-2)+k+ 25*gsize];
			ptrg27[j*(w-2)+k]=ptrGray[j*(w-2)+k+ 26*gsize];
			ptrg28[j*(w-2)+k]=ptrGray[j*(w-2)+k+27*gsize];
			ptrg29[j*(w-2)+k]=ptrGray[j*(w-2)+k+28*gsize];
			ptrg30[j*(w-2)+k]=ptrGray[j*(w-2)+k+29*gsize];
			ptrg31[j*(w-2)+k]=ptrGray[j*(w-2)+k+30*gsize];
		}
	}
	std::cout<<"FIlling last"<<std::endl;
	for(int j=0; j<((h-2)-(((h-2)/32)+1)*31);j++){
		std::cout<<j<<std::endl;
		for(int k=0;k<(w-2);k++){
			if((j*(w-2)+k+31*gsize)<stride && (j*(w-2)+k)<last_size){
				ptrg32[j*(w-2)+k]=ptrGray[j*(w-2)+k+31*gsize];
			}
		}
	}
	std::cout<<"FIlling last"<<std::endl;
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "Gauss_Filter:{Gauss_Filter_1}", &err));
            OCL_CHECK(err, krnl_vector_add2 = cl::Kernel(program2, "Gauss_Filter:{Gauss_Filter_2}", &err));
            OCL_CHECK(err, krnl_vector_add3 = cl::Kernel(program3, "Gauss_Filter:{Gauss_Filter_3}", &err));
            OCL_CHECK(err, krnl_vector_add4 = cl::Kernel(program4, "Gauss_Filter:{Gauss_Filter_4}", &err));
            OCL_CHECK(err, krnl_vector_add5 = cl::Kernel(program5, "Gauss_Filter:{Gauss_Filter_5}", &err));
            OCL_CHECK(err, krnl_vector_add6 = cl::Kernel(program6, "Gauss_Filter:{Gauss_Filter_6}", &err));
            OCL_CHECK(err, krnl_vector_add7 = cl::Kernel(program7, "Gauss_Filter:{Gauss_Filter_7}", &err));
            OCL_CHECK(err, krnl_vector_add8 = cl::Kernel(program8, "Gauss_Filter:{Gauss_Filter_8}", &err));
            OCL_CHECK(err, krnl_vector_add9 = cl::Kernel(program9, "Gauss_Filter:{Gauss_Filter_9}", &err));
            OCL_CHECK(err, krnl_vector_add10 = cl::Kernel(program10, "Gauss_Filter:{Gauss_Filter_10}", &err));
            OCL_CHECK(err, krnl_vector_add11 = cl::Kernel(program11, "Gauss_Filter:{Gauss_Filter_11}", &err));
            OCL_CHECK(err, krnl_vector_add12 = cl::Kernel(program12, "Gauss_Filter:{Gauss_Filter_12}", &err));
            OCL_CHECK(err, krnl_vector_add13 = cl::Kernel(program13, "Gauss_Filter:{Gauss_Filter_13}", &err));
            OCL_CHECK(err, krnl_vector_add14 = cl::Kernel(program14, "Gauss_Filter:{Gauss_Filter_14}", &err));
            OCL_CHECK(err, krnl_vector_add15 = cl::Kernel(program15, "Gauss_Filter:{Gauss_Filter_15}", &err));
            OCL_CHECK(err, krnl_vector_add16 = cl::Kernel(program16, "Gauss_Filter:{Gauss_Filter_16}", &err));
            OCL_CHECK(err, krnl_vector_add17 = cl::Kernel(program17, "Gauss_Filter:{Gauss_Filter_17}", &err));
            OCL_CHECK(err, krnl_vector_add18 = cl::Kernel(program18, "Gauss_Filter:{Gauss_Filter_18}", &err));
            OCL_CHECK(err, krnl_vector_add19 = cl::Kernel(program19, "Gauss_Filter:{Gauss_Filter_19}", &err));
            OCL_CHECK(err, krnl_vector_add20 = cl::Kernel(program20, "Gauss_Filter:{Gauss_Filter_20}", &err));
            OCL_CHECK(err, krnl_vector_add21 = cl::Kernel(program21, "Gauss_Filter:{Gauss_Filter_21}", &err));
            OCL_CHECK(err, krnl_vector_add22 = cl::Kernel(program22, "Gauss_Filter:{Gauss_Filter_22}", &err));
            OCL_CHECK(err, krnl_vector_add23 = cl::Kernel(program23, "Gauss_Filter:{Gauss_Filter_23}", &err));
            OCL_CHECK(err, krnl_vector_add24 = cl::Kernel(program24, "Gauss_Filter:{Gauss_Filter_24}", &err));
            OCL_CHECK(err, krnl_vector_add25 = cl::Kernel(program25, "Gauss_Filter:{Gauss_Filter_25}", &err));
            OCL_CHECK(err, krnl_vector_add26 = cl::Kernel(program26, "Gauss_Filter:{Gauss_Filter_26}", &err));
            OCL_CHECK(err, krnl_vector_add27 = cl::Kernel(program27, "Gauss_Filter:{Gauss_Filter_27}", &err));
            OCL_CHECK(err, krnl_vector_add28 = cl::Kernel(program28, "Gauss_Filter:{Gauss_Filter_28}", &err));
            OCL_CHECK(err, krnl_vector_add29 = cl::Kernel(program29, "Gauss_Filter:{Gauss_Filter_29}", &err));
            OCL_CHECK(err, krnl_vector_add30 = cl::Kernel(program30, "Gauss_Filter:{Gauss_Filter_30}", &err));
            OCL_CHECK(err, krnl_vector_add31 = cl::Kernel(program31, "Gauss_Filter:{Gauss_Filter_31}", &err));
            OCL_CHECK(err, krnl_vector_add32 = cl::Kernel(program32, "Gauss_Filter:{Gauss_Filter_32}", &err));
            valid_device = true;
    ...
    OCL_CHECK(err, cl::Buffer buffer_in62(context31, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, gsize_b,
                                         ptrg31, &err));
    OCL_CHECK(err, cl::Buffer buffer_in63(context32, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, rgbSize,
                                         ptrRGB, &err));
    OCL_CHECK(err, cl::Buffer buffer_in64(context32, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, last_size_b,
                                         ptrg32, &err));
	...
	for (int j=0; j<(((h-2)/32)+1); j++){
		for(int k=0; k<(w-2);k++){
			ptrGray[j*(w-2)+k]=ptrg1[j*(w-2)+k];
			ptrGray[j*(w-2)+k+ gsize]=ptrg2[j*(w-2)+k];
			ptrGray[j*(w-2)+k+ 2*gsize]=ptrg3[j*(w-2)+k];
			ptrGray[j*(w-2)+k+3*gsize]=ptrg4[j*(w-2)+k];
			ptrGray[j*(w-2)+k+4*gsize]=ptrg5[j*(w-2)+k];
			ptrGray[j*(w-2)+k+5*gsize]=ptrg6[j*(w-2)+k];
			ptrGray[j*(w-2)+k+6*gsize]=ptrg7[j*(w-2)+k];
			ptrGray[j*(w-2)+k+7*gsize]=ptrg8[j*(w-2)+k];
			ptrGray[j*(w-2)+k+8*gsize]=ptrg9[j*(w-2)+k];
			ptrGray[j*(w-2)+k+ 9*gsize]=ptrg10[j*(w-2)+k];
			ptrGray[j*(w-2)+k+ 10*gsize]=ptrg11[j*(w-2)+k];
			ptrGray[j*(w-2)+k+11*gsize]=ptrg12[j*(w-2)+k];
			ptrGray[j*(w-2)+k+12*gsize]=ptrg13[j*(w-2)+k];
			ptrGray[j*(w-2)+k+13*gsize]=ptrg14[j*(w-2)+k];
			ptrGray[j*(w-2)+k+14*gsize]=ptrg15[j*(w-2)+k];
			ptrGray[j*(w-2)+k+15*gsize]=ptrg16[j*(w-2)+k];
			ptrGray[j*(w-2)+k+16*gsize]=ptrg17[j*(w-2)+k];
			ptrGray[j*(w-2)+k+ 17*gsize]=ptrg18[j*(w-2)+k];
			ptrGray[j*(w-2)+k+ 18*gsize]=ptrg19[j*(w-2)+k];
			ptrGray[j*(w-2)+k+19*gsize]=ptrg20[j*(w-2)+k];
			ptrGray[j*(w-2)+k+20*gsize]=ptrg21[j*(w-2)+k];
			ptrGray[j*(w-2)+k+21*gsize]=ptrg22[j*(w-2)+k];
			ptrGray[j*(w-2)+k+22*gsize]=ptrg23[j*(w-2)+k];
			ptrGray[j*(w-2)+k+23*gsize]=ptrg24[j*(w-2)+k];
			ptrGray[j*(w-2)+k+24*gsize]=ptrg25[j*(w-2)+k];
			ptrGray[j*(w-2)+k+ 25*gsize]=ptrg26[j*(w-2)+k];
			ptrGray[j*(w-2)+k+ 26*gsize]=ptrg27[j*(w-2)+k];
			ptrGray[j*(w-2)+k+27*gsize]=ptrg28[j*(w-2)+k];
			ptrGray[j*(w-2)+k+28*gsize]=ptrg29[j*(w-2)+k];
			ptrGray[j*(w-2)+k+29*gsize]=ptrg30[j*(w-2)+k];
			ptrGray[j*(w-2)+k+30*gsize]=ptrg31[j*(w-2)+k];
		}
	}
	std::cout<<"FIlling last"<<std::endl;
	for(int j=0; j<((h-2)-(((h-2)/32)+1)*31);j++){
		...
		for(int k=0;k<(w-2);k++){
			if((j*(w-2)+k+31*gsize)<stride && (j*(w-2)+k)<last_size){
				ptrGray[j*(w-2)+k+31*gsize]=ptrg32[j*(w-2)+k];
			}
		}
	}
    // OPENCL HOST CODE AREA END
    CImgDisplay dispGray(imgGrayScale, "FPGA Gray Scale");
    imgGrayScale.save_jpeg("/home/sureshm/Senior_Design_HPC/GPU/ImageProcessing/Gray_Scale/blur_power.jpg");
